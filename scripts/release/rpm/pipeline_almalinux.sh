...
docker build \
    ...
    -f ./scripts/release/rpm/Dockerfile.almalinux \
    -t azure/azure-cli:almalinux8-builder \
    .
...
docker save azure/azure-cli:almalinux8-builder | gzip > ${BUILD_STAGINGDIRECTORY}/docker/azure_azure-cli_almalinux8-builder.tar.gz &
docker save azure/azure-cli:almalinux8 | gzip > ${BUILD_STAGINGDIRECTORY}/docker/azure_azure-cli_almalinux8.tar.gz &
wait

### Remove me this is for reference for centos build rpm 
# RUN yum install -y rpm-build zlib-devel nspr-devel nss-devel file-devel popt-devel libarchive-devel libdb-devel epel-release lua-devel
# RUN yum groupinstall -y 'Development Tools'
# RUN mkdir -p /rpmsource && \
#     curl -L --silent https://ftp.osuosl.org/pub/rpm/releases/rpm-4.14.x/rpm-4.14.3.tar.bz2 -o /rpmsource/rpm-4.14.3.tar.bz2 && \
